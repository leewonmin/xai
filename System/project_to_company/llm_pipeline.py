...
SYSTEM_PROMPT = (
    ...
    "  [현재 과제와의 유사성]\n"
    "   - company.has_patent_matching_related가 true인 경우에만, patent_matching_related에 포함된 특허 제목들과 2)에서 파악한 현재 과제의 내용을 비교하여 기술적 연관성을 설명한다.\n"
    ...
    "    - 이후 해당 기술이 일반적으로 어떤 장치나 시스템에서 사용되는지 설명하고, 그 장치 또는 시스템이 1)에서 파악한 회사 사업과 어떻게 연결되는지 설명한다.\n"
    ...
    "   - 해당 과제를 실제로 수행했던 회사들의 company_info와 추천된 회사인 1)에서 파악한 내용과 현재 과제인 2)에서 파악한 내용을 비교하여 세 요소가 공통적으로 어떤 기술, 사업, 공정, 제품, 연구개발 방향에서 유사한지 설명한다.\n"
    ...
    "   - region_relation.has_same_region이 true이고 region_relation.has_similar_region도 true이면 동일 지역 근거를 보조적으로 사용하고, company.region 값을 직접 언급한다.\n"
    "   - region_relation.has_same_region이 false이고 region_relation.has_similar_region도 false이면 지역적 연관성 관련 내용을 어떠한 형태로도 작성하지 않는다.\n"
    ...
    "   - 추천된 회사가 과거에 수행했던 과제들의 project_info와 현재 과제인 2)에서 파악한 과제 내용을 비교하여 목표, 핵심 기술, 적용 방식, 해결하려는 문제 측면에서 어떤 유사성이 있는지 설명한다.\n"
...
FEWSHOT_MESSAGES = [
    ...
    {"role": "assistant", "content": json.dumps({
        "연관성": (
                    "액정 엘라스토머 기반 4D 프린팅 소재 개발 과제는 용액공정, 코팅공정, 잉크토출장치와 같은 정밀 공정 기술을 기반으로 기능성 소재를 안정적으로 형성하는 기술이 핵심이다."
                    "액정 엘라스토머와 같은 기능성 소재는 온도, 자극, 표면 상태에 따라 형태 변화 특성이 달라질 수 있기 때문에, 코팅 두께나 소재 분산 상태가 불균일하면 원하는 변형 특성이 저하될 수 있어 정밀한 용액 기반 공정과 패터닝 기술이 필요하다."
                    "샘플회사_알파12a는 코팅공정용 장비, 용액기반 공정기술, 잉크토출장치 및 금속입자소결체 관련 기술 요소를 보유하고 있어 이러한 소재 제조 과정에서 요구되는 정밀 코팅과 패터닝 공정에 연계될 가능성이 높다."
                    "특히 용액을 균일하게 분사하고 제어하는 기술은 기능성 필름이나 프린팅 기반 소재 제작 과정에서 중간 제조 공정으로 활용될 수 있으며, 이는 액정 엘라스토머 기반 구조체 제작과도 연결될 수 있다." 
                    "또한 관련 연구 및 특허에서 나타나는 액정 엘라스토머 필름 제조와 4D 프린팅 기술 방향은 샘플회사_알파12a의 코팅·프린팅 기반 공정 기술과 기술적 접점이 높아 과제 수행 과정에서 적용 가능성이 있는 것으로 판단된다.",
        ),
        "추천 기업의 우수성": (
                    "현재 과제는 액정 엘라스토머 및 4D 프린팅 관련 논문 1건과 특허 1건의 연구 성과를 기반으로 하고 있어 기능성 소재 제어와 프린팅 공정 기술 측면에서 일정 수준 이상의 기술 성숙도를 갖춘 과제로 볼 수 있다."
                    "추천된 샘플회사_알파12a는 ASTI 회원사이면서 특구 지정기업과 벤처기업, 이노비즈 인증을 보유하고 있어 이는 기술 혁신성과 사업화 역량을 동시에 보유한 기업이라는 점에서 과제 수행 적합성을 높이는 요소로 작용할 수 있다."
                    "또한 매출성장율이 전체의 상위 5% 수준이며 연구개발비가 전문직별 공사업 업종 내 상위 20% 수준으로 나타나 지속적인 기술 투자와 성장 기반을 갖춘 기업으로 해석할 수 있다."
                    "부채비율 역시 전체의 상위 10% 수준으로 안정적인 재무 구조를 유지하고 있어 연구개발 과제를 수행하기 위한 운영 안정성 측면에서도 긍정적인 요소가 있다."
                    "과제 자체도 총연구비가 전체의 상위 5% 수준으로 대형 연구개발 성격을 가지며, 기능성 소재와 정밀 프린팅 공정을 융합하는 기술적 파급력이 높다는 점에서 우수성이 있다."
                    "이러한 요소를 종합하면 샘플회사_알파12a는 정밀 코팅 및 프린팅 기반 공정 기술 역량과 안정적인 성장 기반을 바탕으로 해당 과제와의 연계 가능성이 높은 기업으로 판단된다.",
       ),
        "유사 사례": (
                    "샘플회사_알파12a의 특허 매칭 사례에서는 프린팅 장치, 전기수력학 기반 분사 노즐, 피드백 제어형 인쇄 시스템과 같이 정밀 분사와 패터닝 제어를 중심으로 한 기술들이 공통적으로 확인된다."
                    "이러한 기술은 기능성 소재를 균일하게 분사하거나 미세 패턴으로 형성해야 하는 공정과 연결될 수 있으며, 액정 엘라스토머 기반 4D 프린팅 소재 제작 과정에서도 소재 배열과 코팅 균일성을 확보하기 위한 핵심 기술 요소로 활용될 수 있다."
                    "또한 현재 과제와 연관된 논문 및 특허에서는 기능성 필름 제조, 탄성 소재 제어, 4D 프린팅 기반 구조 형성 기술이 공통적으로 나타나며, 이는 정밀 코팅 및 프린팅 공정 기술이 필요한 대표적인 응용 분야로 볼 수 있다."
                    "이러한 기술은 기능성 필름, 전자소자용 소재, 스마트 구조체 제작 과정에서 활용될 수 있으며, 샘플회사_알파12a의 코팅 장비 및 용액 공정 기술과 자연스럽게 연결될 수 있다."
                    "과거 수행 기업 사례에서도 기능성 필름, 정밀 코팅, 박막 형성, 공정 제어와 같은 기술 방향이 반복적으로 나타나 현재 과제와 공통적으로 정밀 소재 제조 공정에 집중하고 있음을 확인할 수 있다."
                    "특히 서울 지역 수행 기업 사례가 함께 존재해 샘플회사_알파12a와의 지역적 연관성도 보조적으로 작용할 수 있으며, 회사가 과거 수행한 기능성 필름 제조 공정 및 정밀 프린팅 기반 소재 패터닝 과제 역시 현재 과제와 유사한 연구개발 방향성을 보여준다."
      )
    }, ensure_ascii=False)}
]
