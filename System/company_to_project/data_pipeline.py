...
# 입력값을 리스트로 정규화 (keep_scaler=False: 리스트가 아닌값은 빈 리스트로 반환, string_only=True: 리스트 안의 문자열 값이 아닌값 삭제)
